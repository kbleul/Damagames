

export const Localization = {
    "Login": {
        AMH: "ይግቡ",
        ENG: "Log in"
    },
    "Create Game": {
        AMH: "አዲስ ጨዋታ",
        ENG: "Create Game"
    },
    "Join Game": {
        AMH: "ጨዋታ ይቀላቀሉ",
        ENG: "Join Game"
    },
    "Play With Computer": {
        AMH: "ከኮምፒውተር ጋር ይጫወቱ",
        ENG: "Play with Computer"
    },
    "Public Game": {
        AMH: "የጋራ ጨዋታ",
        ENG: "Public Game"
    },
    "Forgot Password": {
        AMH: "ይለፍቃል ረስተዋል ?",
        ENG: "Forgot Password"
    },
    "password": {
        AMH: "ይለፍቃል",
        ENG: "Password"
    },
    "Don't have account": {
        AMH: "መለያ የሎትሞ ?",
        ENG: "Don't have account ?"
    },
    "Sign up": {
        AMH: "ይመዝገቡ",
        ENG: "Sign up"
    },
    "please fill the space !": {
        AMH: "እባክዎን መልሶን ያስገቡ !",
        ENG: "please fill the space !"
    },
    "please enter a valid phone !": {
        AMH: "እባክዎ ትክክለኛ ስልክ ያስገቡ !",
        ENG: "please enter a valid phone !"
    },
    "please enter a strong password !": {
        AMH: "እባክዎ ጠንካራ የይለፍ ቃል ያስገቡ !",
        ENG: "please enter a strong password !"
    },
    "Coins earned": {
        AMH: "ሳንቲሞች",
        ENG: "Coins earned"
    },
    "Games played": {
        AMH: "ጨዋታዎች",
        ENG: "Games played"
    },
    "Wins": {
        AMH: "ድል",
        ENG: "Wins"
    },
    "Draw": {
        AMH: "አቻ",
        ENG: "Draw"
    },
    "Loss": {
        AMH: "ሽንፈት",
        ENG: "Loss"
    },
    "Chat": {
        AMH: "መልእክት",
        ENG: "Chat"
    },
    "Profile": {
        AMH: "መገለጫ",
        ENG: "Profile"
    },
    "Log out": {
        AMH: "ይዉጡ",
        ENG: "Log out"
    },
    "Score board": {
        AMH: "የውጤት ሰሌዳ",
        ENG: "Score board"
    },
    "Store": {
        AMH: "ገበያ",
        ENG: "Store"
    },
    "Tell us your name": {
        AMH: "ስሞን ይንገሩን",
        ENG: "Tell us your name"
    },
    "Create": {
        AMH: "ያስጀምሩ",
        ENG: "Create"
    },
    "Creating": {
        AMH: "በማስጀመር ላይ ...",
        ENG: "Creating ..."
    },
    "Play for coin": {
        AMH: "ለሳንቲም ይጫወቱ",
        ENG: "Play for coin"
    },
    "Great work": {
        AMH: "ጎበዝ !",
        ENG: "Great work !"
    },
    "Now send this": {
        AMH: "አሁን ይህንን ሊንክ ለጓደኛዎ ይላኩ ።",
        ENG: "Now send this Link to your Friend"
    },
    "Copy": {
        AMH: "ገልብጥ",
        ENG: "Copy"
    },
    "Copied": {
        AMH: "ተገልብጧል",
        ENG: "Copied"
    },
    "Share": {
        AMH: "አጋራ",
        ENG: "Share"
    },
    "you are about to": {
        AMH: "ይህን ጨዋታ ለቀዉ ሊሄዱ ነው ።",
        ENG: "You are about to leave this game !"
    },
    "Yes": {
        AMH: "አዎ",
        ENG: "Yes"
    },
    "No": {
        AMH: "አይ",
        ENG: "No"
    },
    "Are you sure_": {
        AMH: "እርግጠኛ ኖት መውጣት ይፈልጋሉ ?",
        ENG: "Are you sure you want to leave ?"
    },
    "Are you sure": {
        AMH: "እርግጠኛ ኖት መውጣት ይፈልጋሉ ? አሁን ከሄዱ ጓደኛዎ ይህን ጨዋታ መቀላቀል አይችሉም !",
        ENG: "Are you sure you want to leave ? Your friend won't be able to join this game if you leave now !"
    },
    "sound on": {
        AMH: "ድምጽ በርቷል",
        ENG: "sound on"
    },
    "sound off": {
        AMH: "ድምፅ ጠፍቷል",
        ENG: "sound off"
    },
    "exit": {
        AMH: "መውጫ",
        ENG: "exit"
    },
    "Computer": {
        AMH: "ኮምፒውተር",
        ENG: "Computer"
    },
    "Rematch": {
        AMH: "ድጋሜ ይግጠሙ",
        ENG: "Rematch"
    },
    "New Game": {
        AMH: "አዲስ ጨዋታ",
        ENG: "New Game"
    },
    "Congratulations": {
        AMH: "እንኳን ደስ አሎት !",
        ENG: "Congratulations !"
    },
    "Previous": {
        AMH: "የበፊት",
        ENG: "Previous"
    },
    "Total": {
        AMH: "ጠቅላላ",
        ENG: "Total"
    },
    "Your turn": {
        AMH: "የእርሶ ተራ",
        ENG: "Your turn"
    },
    "Moves": {
        AMH: "እንቅስቃሴ",
        ENG: "Moves"
    },
    "Pawns": {
        AMH: "ቆርኪዎች",
        ENG: "Pawns"
    },
    "Your coins": {
        AMH: "የእርስዎ ሳንቲሞች",
        ENG: "Your coins"
    },
    "Play for coins": {
        AMH: "ለሳንቲሞች ይጫወቱ",
        ENG: "Play for coins"
    },
    "Great Work": {
        AMH: "በጣም ጥሩ !",
        ENG: "Great Work"
    },
    "enter code": {
        AMH: "ሚስቲር ቁጥር ያስገቡ",
        ENG: "Enter code"
    },
    "Submit": {
        AMH: "ያስገቡ",
        ENG: "Submit"
    },
    "Or": {
        AMH: "ወይም",
        ENG: "Or"
    },
    "Log in to bet coins": {
        AMH: "በሳንቲም ለመጫወት ወደ አካዉንቶ ይግቡ ",
        ENG: "Log in to bet coins"
    },
    "Bet": {
        AMH: "ውርርድ",
        ENG: "Bet"
    },
    "Here": {
        AMH: "እዚ",
        ENG: "Here"
    },
    "Loading": {
        AMH: "በመከፈት ...",
        ENG: "Loading ..."
    },
    "Your Friend": {
        AMH: "ጓደኛዎ",
        ENG: "Your Friend"
    },
    "waiting for you.": {
        AMH: " እየጠበቆት ነው::",
        ENG: " is waiting for you."
    },
    "Join Now !!": {
        AMH: "አሁን ይቀላቀሉ !!",
        ENG: "Join Now !!"
    },
    "Join": {
        AMH: "ተቀላቀል",
        ENG: "Join"
    },
    "Avatars": {
        AMH: "የመገለጫ ምስል",
        ENG: "Avatars"
    },
    "Boards": {
        AMH: "የመጫወቻ ሰሌዳ",
        ENG: "Boards"
    },
    "Crowns": {
        AMH: "ዘውዶች",
        ENG: "Crowns"
    },
    "view all": {
        AMH: "ሁሉንም አሳይ",
        ENG: "view all"
    },
    "You need to login to buy items.": {
        AMH: "ለመግዛት በመለያ መግባት አለብዎት.",
        ENG: "You need to login to buy items."
    },
    "Buy Now": {
        AMH: "ግዛ",
        ENG: "Buy Now"
    },
    "Purchased": {
        AMH: "የተገዛ",
        ENG: "Purchased"
    },
    "You don't have sufficient coins": {
        AMH: "በቂ ሳንቲሞች የሎትም። ሳንቲሞችን ይግዙ ወይም ተጨማሪ ጨዋታዎችን ይጫወቱ.",
        ENG: "You don't have sufficient coins"
    },
    "Play Game": {
        AMH: "ይጫወቱ",
        ENG: "Play Game"
    },
    "Buy Coins": {
        AMH: "ሳንቲሞችን ይግዙ",
        ENG: "Buy Coins"
    },
    "Select Coin": {
        AMH: "ሳንቲም ይምረጡ",
        ENG: "Select Coin"
    },
    "coins": {
        AMH: " ሳንቲሞች",
        ENG: " coins"
    },
    "You Lost !": {
        AMH: "ተሸንፈዋል !",
        ENG: "You Lost !"
    },
    "You won 0 coins.": {
        AMH: "0 ሳንቲም አሸንፈዋል።",
        ENG: "You won 0 coins."
    },
    "write your message...": {
        AMH: " መልእክቶን ይፃፉ",
        ENG: "write your message..."
    },
    "birr": {
        AMH: " ብር",
        ENG: "birr"
    },
    "Buy": {
        AMH: "ግዛ",
        ENG: "Buy"
    },
    "Amount": {
        AMH: "መጠን",
        ENG: "Amount"
    },
    "Pay with": {
        AMH: "ይክፈሉ",
        ENG: "Pay with"
    },
    "Payment Successful!": {
        AMH: "ክፍያዉ ተሳክቷል! ስለግዢዎ እናመሰግናለን",
        ENG: "Payment Successful! Thank you for your purchase"
    },
    "Go to Shop": {
        AMH: "ወደ ገበያ ይሂዱ",
        ENG: "Go to Shop"
    },
    "Change Password": {
        AMH: "የይለፍ ቃል ይቀይሩ",
        ENG: "Change Password"
    },
    "Language": {
        AMH: "ቋንቋ",
        ENG: "Language"
    },
    "My Boards": {
        AMH: "የእርሶ ሰሌዳዎች",
        ENG: "My Boards"
    },
    "My Crowns": {
        AMH: "የእርሶ ዘውዶች",
        ENG: "My Crowns"
    },
    "Shop": {
        AMH: "ወደ ገበያ ይሂዱ",
        ENG: "Shop"
    },
    "You don't have any boards yet.": {
        AMH: "እስካሁን ምንም ሰሌዳ የሎትም።",
        ENG: "You don't have any boards yet."
    },
    "You don't have any crowns yet.": {
        AMH: "እስካሁን ምንም ዘውዶች የሎትም።",
        ENG: "You don't have any crowns yet."
    },
    "Go to the store to buy one": {
        AMH: "ለመግዛት ወደ መደብሩ ይሂዱ",
        ENG: "Go to the store to buy one"
    },
    "Username": {
        AMH: "የተጠቃሚ ስም",
        ENG: "Username"
    },
    "Change Username": {
        AMH: "የተጠቃሚ ስም የሎትም",
        ENG: "Change Username"
    },
    "Username changed.": {
        AMH: "የተጠቃሚ ስም ተቀይሯል።",
        ENG: "Username changed."
    },
    "Password changed.": {
        AMH: "የይለፍ ቃል ተቀይሯል።",
        ENG: "Password changed."
    },
    "Confirm Password": {
        AMH: "የይለፍ ቃል ያረጋግጡ",
        ENG: "Confirm Password"
    },
    "Current Password": {
        AMH: "የአሁን የይለፍ ቃል",
        ENG: "Current Password"
    },
    "New Password": {
        AMH: "አዲሱን የይለፍ ቃል",
        ENG: "New Password"
    },
    "Update default board": {
        AMH: "የመጫወቻ ሰሌዳ ይቀይሩ",
        ENG: "Update default board"
    },
    "Update default crown": {
        AMH: "የመጫወቻ ዘውድ ይቀይሩ ",
        ENG: "Update default crown"
    },
    "Board changed.": {
        AMH: "የመጫወቻ ሰሌዳ ተለውጧል።",
        ENG: "Board changed."
    },
    "Crown changed.": {
        AMH: "የመጫወቻ ዘውድ ተለውጧል::",
        ENG: "Crown changed."
    },
    "No public games currently !": {
        AMH: "በአሁኑ ጊዜ ምንም የጋራ ጨዋታዎች የሉም ::",
        ENG: "No public games available right now !"
    },
    "Play": {
        AMH: "ተጫወት",
        ENG: "Play"
    },
    "someone has already": {
        AMH: "ሌላ ሰው አስቀድሞ ጨዋታውን ተቀላቅሏል።",
        ENG: "someone has already joined the game"
    },
    "Back": {
        AMH: "ተመለስ",
        ENG: "Back"
    },
    "Close": {
        AMH: "ዝጋ",
        ENG: "Close"
    },
    "Update": {
        AMH: "ለዉጥ",
        ENG: "Update"
    },
    "SoundOn": {
        AMH: "ድምጽ በርቷል",
        ENG: "SoundOn"
    },
    "SoundOff": {
        AMH: "ድምጽ ጠፍቷል",
        ENG: "SoundOff"
    },
    "Timer": {
        AMH: "ሰዓት ቆጣሪ",
        ENG: "Timer"
    },
    "You will lose": {
        AMH: "ቀጥለህ ካልተንቀሳቀስክ ትሸነፋለህ",
        ENG: "You will lose if you don't move next"
    },
    "Exit": {
        AMH: "ውጣ",
        ENG: "Exit"
    },
    "Leave": {
        AMH: "ውጣ",
        ENG: "Leave"
    },
    "Accept": {
        AMH: "ተቀበል",
        ENG: "Accept"
    },
    "Reject": {
        AMH: "አይ",
        ENG: "Reject"
    },
    "Your friend has left": {
        AMH: "ጓደኛዎ ጨዋታውን ትቶ ወጥአል ።",
        ENG: " Your friend has left out the game !"
    },
    "You can't continue playing": {
        AMH: "ይቅርታ ጨዋታውን መጫወት መቀጠል አይችሉም !",
        ENG: "You can't continue playing the game !"
    },
    "your friend has requested": {
        AMH: "ጓደኛዎ በድጋሚ ለጫወት ጠይቆቷል",
        ENG: "your friend has requested for a rematch ?"
    },
    "You friend did not accept the request !": {
        AMH: "ጓደኛዎ ጥያቄውን አልተቀበለም !",
        ENG: "You friend did not accept the request !"
    },
    "You friend has requested for a draw ?": {
        AMH: "ጓደኛዎ አቻ ለመዉጣት ጠይቀዋል",
        ENG: "You friend has requested for a draw ?"
    },
    "Waiting for your friend": {
        AMH: "ጓደኛዎ እስከሚቀላቀሉ...",
        ENG: "Waiting for your friend"
    },
    "Fetch store items failed.": {
        AMH: "የገበያ ዕቃዎችን ማምጣት አልተሳካም። እንደገና ይሞክሩ።",
        ENG: "Fetch store items failed."
    },
    "Enter your Phone and we'll send": {
        AMH: "እባኮትን ስልኮትን ያስገቡ በመቀጠል የይለፍ ቃል ማስተካከያ መመረጃ እንልክሎታለን።",
        ENG: "Enter your Phone and we'll send you instructions to reset your password"
    },
    "Enter Your New Password": {
        AMH: "አዲሱን የይለፍ ቃልዎን ያስገቡ",
        ENG: "Enter Your New Password"
    },
    "Your OTP Code:": {
        AMH: "የእርስዎ የሚስጥር ቁጥር",
        ENG: "Your OTP Code:"
    },
    "OTP is sent to your phone": {
        AMH: "የሚስጥር ቁጥር ወደ ስልክህ ተልኳል።",
        ENG: "OTP is sent to your phone"
    },
    "Phone no": {
        AMH: "ስልክ ቁጥር",
        ENG: "Phone no"
    },
    "Send": {
        AMH: "ላክ",
        ENG: "Send"
    },
    "success": {
        AMH: "ተሳክቷል",
        ENG: "success"
    },
    "SUCCESS": {
        AMH: "ተሳክቷል",
        ENG: "success"
    },
    "ERROR": {
        AMH: "አልተሳካሞ",
        ENG: "ERROR"
    },
    "Re-Send": {
        AMH: "እንደገና ላክ",
        ENG: "Re-Send"
    },
    "Back to login": {
        AMH: "ወደ መግቢያ ተመለስ",
        ENG: " Back to login"
    },
    "Rank": {
        AMH: "ደረጃ",
        ENG: "Rank"
    },
    "Enter Your username and password": {
        AMH: "የተጠቃሚ ስሞን እና የይለፍ ቃሎን ያስገብ",
        ENG: "Enter Your username and password"
    },
    "Already have account ?": {
        AMH: "የግል መለያ አለዎት ?",
        ENG: "Already have account ?"
    },
    "name is required.": {
        AMH: "ስምዎን ማስገባት ኣስፈላጊ ነው !",
        ENG: "name is required !"
    },
    "Amount has to be less than your coins": {
        AMH: "መጠኑ ከእርስዎ ሳንቲሞች ያነሰ መሆን አለበት ።",
        ENG: "Amount has to be less than your coins"
    },
    "Invalid Amount": {
        AMH: "ያልተፈቀደ መጠን",
        ENG: "Invalid Amount"
    },
    "code character should be 6": {
        AMH: "የሚስቲር ቁጥር ብዛት 6 መሆን አለበት",
        ENG: "code character should be 6"
    },
    "please fill your answer": {
        AMH: "እባክዎን መልስዎን ይሙሉ",
        ENG: "please fill your answer"
    },
    "profile changed successfully": {
        AMH: "የመለያዎ ምስል በተሳካ ሁኔታ ተቀይሯል",
        ENG: "profile changed successfully"
    },
    "current password is required": {
        AMH: "የአሁኑ የይለፍ ቃል ያስፈልጋል",
        ENG: "current password is required"
    },
    "new password is required": {
        AMH: "አዲስ የይለፍ ቃል ያስፈልጋል",
        ENG: "new password is required"
    },
    "new password does not match": {
        AMH: "አዲስ የይለፍ ቃል አይዛመድም።",
        ENG: "የማረጋገጫ ይለፍ ቃል ከዓዲሱ ይለፍ ካል ጋር ኣይመሳሰልም፥፥ አባክዎ ኣስተካክለው ያስገቡ"
    },
    "Username is too short !": {
        AMH: "የመለያ ስምዎ ኣጭር ነው !",
        ENG: "Username is too short !"
    },
    "please enter PhoneNo": {
        AMH: "እባክዎን ስልክ ቁጥሮዎን ያስገቡ",
        ENG: "please enter Phone number"
    },
    "Verification Code must be 4 digits": {
        AMH: "የማረጋገጫ ሚስቲር ቁጥር 4 አሃዝ መሆን አለበት።",
        ENG: "Verification Code must be 4 digits"
    },
    "Verification Code must 6 digits": {
        AMH: "የማረጋገጫ ሚስቲር ቁጥር 6 አሃዝ መሆን አለበት።",
        ENG: "Verification Code must 6 digits"
    },
    "please enter username": {
        AMH: "እባክዎ የመለያ ስምዎን ያስገቡ",
        ENG: "please enter username"
    },
    "please enter password": {
        AMH: "እባክዎ የይለፍ ቃል ያስገቡ",
        ENG: "please enter password"
    },
    "password does not match": {
        AMH: "ያስገቡት የይለፍ ቃል አይዛመድም።",
        ENG: "password does not match"
    },
    "phone number starts with 9": {
        AMH: "ስልክ ቁጥር በ9 ይጀምራል እና የቁትሮችሁ ብዛትም 9 መሆን ኣለበት",
        ENG: "phone number starts with 9"
    },
    "password is required": {
        AMH: "የይለፍ ቃል ያስፈልጋል",
        ENG: "password is required"
    },
    "phone is required": {
        AMH: "ስልክ ቁጥሮዎ ያስፈልጋል",
        ENG: "phone is required"
    },
    "Login successful!": {
        AMH: "መግባት ተሳክቷል ።",
        ENG: "Login successful !"
    },
    "password has been successfully changed": {
        AMH: "የይለፍ ቃል በተሳካ ሁኔታ ተቀይሯል ።",
        ENG: "password has been successfully changed"
    },
    "Game created successfully!": {
        AMH: "ጨዋታው በተሳካ ሁኔታ ተፈጥሯል።",
        ENG: "Game created successfully!"
    },
    "Waiting for someone to": {
        AMH: "ተጋጣሚዎ አስኪቀላቀል በመጠበቅ ላይ ",
        ENG: "Waiting for someone to join your game."
    },
    "Undo": {
        AMH: "መመለስ",
        ENG: "Undo"
    },
    "Redo": {
        AMH: "ወደፊት",
        ENG: "Redo"
    },
    "Undo limit reached.": {
        AMH: "መመለስ ገደብ ላይ ደርሷል። ተጨማሪ እንቅስቃሴዎችን መመለስ አይችሉም።",
        ENG: "Undo limit reached. You cannot undo any more moves!"
    },
    "resume computer turn ?": {
        AMH: "የኮምፒተር ዙር",
        ENG: "resume computer turn ?"
    },
    "Resume": {
        AMH: "  ይቀጥል",
        ENG: "  Resume"
    },
    "top": {
        AMH: "ከፍተኛ",
        ENG: "Top"
    },
    "leaders": {
        AMH: "መሪዎች",
        ENG: "Leaders"
    },








    "Dama": {
        AMH: "ዳማ",
        ENG: "Dama"
    },
    "Start Tour": {
        AMH: "ኣሳይ",
        ENG: "Start Tour"
    },
    "Let’s begin by going": {
        AMH: "የዳማን ኣጨዋወት በማየት ይጀምሩ።",
        ENG: "Let’s begin with a simple tour of Dama."
    },
    "Sharpen your skills with": {
        AMH: "ብቃት ካለው  ኮምፒዩተር ጋር በሚደረግ ፈታኝ ጨዋታ ችሎታዎን ያሳድጉ ፤ የዳማ አጨዋወት ስልትዎን ወደተሻለ ደረጃ ያሳድጉ።",
        ENG: "Sharpen your skills with a challenging game against a skilled AI opponent."
    },
    "Start your own exciting": {
        AMH: "ጨዋታውን በመፍጠር ማስፈንጠሪያውን ወይም ኮዱን ለጓደኛዎ ይላኩ። የራስዎን አስደሳች የዳማ ጨዋታ ይጀምሩ እና ጓደኛዎን በመጋበዝ ይወዳደሩ።",
        ENG: "Start your own exciting dama game and challenge your friends or other players around the world! Just create the game and send the link or code to your friend."
    },
    "Join a game your friend created": {
        AMH: "ጓደኛዎ የፈጠረውን ጨዋታ ይቀላቀሉ እና የፉክክርን ስሜት ይለማመዱ። ችሎታዎን ከሌሎች የተካኑ ተጫዋቾች ጋር ፈትነው የራስዎን ደረጃ ያሻሽሉ።",
        ENG: "Join a game your friend created and experience the thrill of competition."
    },
    "Dive into the action with": {
        AMH: "ወደ ጋራ ጨዋታ በመግባት በዓለም ዙሪያ ከሚገኙ ተጫዋቾች ጋር በመጫወት አስደሳች ጊዜን  ያሳልፉ።",
        ENG: "Dive into the action with a public dama game ! Play against random opponents from diverse backgrounds.",
    },
    "Keep track of your": {
        AMH: "የደረጃ ሰንጠረዥ ዉስጥ በመግባት የእርስዎን ደረጃ ከሌሎች ጋር ያስተያዩ።",
        ENG: "Keep track of your progress and compare your skills with other players on the scoreboard."
    },
    "Customize your game with": {
        AMH: "መጫወቻዎችን ፣ ሰሌዳዎችን አና ዘውዶችን ከገበያ በመግዛት የግል መለያዎትን ያሳምሩ።",
        ENG: "Customize your game with unique pawns, boards, and crowns from the store."
    },
    "Join the community of dama": {
        AMH: "መለያ በመፍጠር ወይም በመለያ በመግባት ዳማን ይቀላቀሉ። ልዩ ልዩ ባህሪያትን ይክፈቱ፣ የርሶን መለያ ያበጁ እና አስደሳች የዳማ ጉዞ ይጀምሩ።",
        ENG: "Join the community of dama enthusiasts by creating an account or logging in. Unlock exclusive features, customize your profile"
    },


    "Select language preference": {
        AMH: "የቋንቋ ምርጫዎን ይምረጡ።",
        ENG: "Select your language preference"
    },

    "Select": {
        AMH: "ምረጥ",
        ENG: "Select"
    },

    "tour0": {
        AMH: "በቅድሚያ ለማንቀሳቀስ እና ሊኖሩ ስለሚችሉ እንቅስቃሴዎች ፍንጮችን ለማየት የእርስዎን መጫወቻ ይንኩ።",
        ENG: "click your pawn to make a move and view hints on possible moves."
    },
    "tour1": {
        AMH: "ከዚያ ለመንቀሳቀስ የመረጡትን መጫወቻ ሳጥን በመንካት መጫወቻዎን ያንቀሳቅሱ።",
        ENG: "Then click the box you want to move to inorder to make a move."
    }


}