import { getMoves } from "./ReactCheckers";


export function getComputerMoves(columns,boardState, player) {
    ...
    for (const coordinates in boardState) {
        ...
        const pieceMoves = getMoves(columns,boardState, coordinates, boardState[coordinates].isKing, false);
...
export function  getSmartMove(columns,state, boardState, player) {
    const computerMoves = getComputerMoves(columns,boardState, player);
    ...
    for (let m = 0; m < moveKeys.length ; ++m) {
        ...
        for (let a = 0; a < movesData.length ; ++a) {
            ...
            while (stateLeaf.currentPlayer === false) {
                ...
                const newJumpMoves = getMoves(columns,stateLeaf, stateLeaf.activePiece, stateLeaf.activePiece.isKing, true);
...
export function  getRandomMove(columns,boardState, player) {
    const computerMoves = getComputerMoves(columns,boardState, player);
