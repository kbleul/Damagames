...
const Avatar = ({ avatar, set_isShowModalOpen, setSelectedItem, setShowLoginModal, myAvatarsId }) => {

    const { user, token } = useAuth();

    return (<article className="flex items-center  w-[90%] ml-[5%] border  p-2 rounded-md mb-4 relative hover:opacity-80 focus:opacity-80"
        style={{
            background: `linear-gradient(120deg, rgb(39, 138, 134) 1%, rgba(11, 42, 43, 0.32) 10%, rgb(22, 85, 82) 98%) repeat scroll 0% 0%`,
        }}
        onClick={() => {
        ...
        <section className="w-[60%] text-sm md:text-lg text-left">
...
const Board = ({ board, isOn, set_isShowModalOpen, setSelectedItem, setShowLoginModal, myBoardsId }) => {

    const { user, token } = useAuth();

    return (<article className="flex flex-col justify-around p-4 pt-1 relative hover:opacity-80 focus:opacity-80"
        onClick={() => {
            if (!user && !token) {
                setShowLoginModal(true)
                return
        ...
        {isOn && <section className=" flex  justify-center items-center text-sm mt-2">
        ...
        {user && token && !myBoardsId.includes(board.id) && <>{parseInt(user.coin) < board.price &&
            <RiLock2Fill className="absolute top-2 right-2 text-orange-color" />
        }

            {(parseInt(user.coin) > board.price || parseInt(user.coin) === board.price) &&
                <ImUnlocked className="absolute top-2 right-2 text-white" />
            }
        </>}
...
const Crown = ({ crown, set_isShowModalOpen, setSelectedItem, setShowLoginModal, isOn, myCrownsId }) => {

    const { user, token } = useAuth();


    return (<article className="hover:opacity-80 focus:opacity-80 mb-6 relative" onClick={() => {
        if (!user && !token) {
            setShowLoginModal(true)
            return
        }
        ...
        <section className={isOn ? "w-4/5 ml-[10%] relative" : "w-4/5 ml-[10%] relative py-2"}>
            ...
            {user && token && !myCrownsId.includes(crown.id) && <>
                ...
                {(parseInt(user.coin) > crown.price || parseInt(user.coin) === crown.price) &&
