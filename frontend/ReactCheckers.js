...
export function getMoves(columns,boardState, coordinates, isKing = false, hasJumped = false) {
    ...
    for (let key in corners) {
      ...
      if (boardState[cornerCoordinates] === null) {
        ...
        let opponentCorners = getCorners(cornerCoordinates);
...
//move piece
export function movePiece(coordinates, gameState) { console.log("pp")
    ...
    console.log(boardState[gameState.activePiece]);
...
console.log("poned")
    ...
    stateOut.winner = evaluateWinner(boardState);
  ...
  export function evaluateWinner(boardState) {
    let player1Pieces = 0;
    let player1Moves = 0;
