...
const BoardTable = ({ avatars, setIsUpdated ,setIsModalOpen ,setEditId}: Props) => {
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "color1",
      headerName: "Board Color",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
        return (
          <div className={` w-10 h-10`} style={{backgroundColor:`${params.row.color.color1}`}}></div>
        );
      },
    },
    {
      field: "color2",
      headerName: "Board Color2",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
        return (
          <div className={` w-10 h-10`} style={{backgroundColor:`${params.row.color.color2}`}}></div>
        );
      },
    },
    {
      field: "lastMoveColor",
      headerName: "lastMoveColor",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
        return (
          <div className={` w-10 h-10`} style={{backgroundColor:`${params.row.color.lastMoveColor}`}}></div>
        );
      },
    },
    {
      field: "board_pawn1",
      headerName: "board_pawn1",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
    ...
    {
      field: "board_pawn1_turn",
      headerName: "board_pawn1_turn",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
        return (
          <img
            src={params.row.board_pawn1_turn}
            alt=""
            className="h-11 w-11 rounded-sm object-cover"
          />
        );
    ...
    {
      field: "board_pawn2_turn",
      headerName: "board_pawn2_turn",
      sortable: false,
      filterable: false,
      width: 150,
      renderCell: (params: GridCellParams) => {
        return (
          <img
            src={params.row.board_pawn2_turn}
            alt=""
            className="h-11 w-11 rounded-sm object-cover"
          />
        );
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              className="bg-main-bg rounded-sm hover:opacity-80
                text-center px-5 p-1 font-medium text-sm text-white"
                onClick={() => navigate(`/board/create/${params.row.id}`)}
            >
              Edit
            </button>
            <Switch
